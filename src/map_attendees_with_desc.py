...
folium.map.Marker(
    ...
    icon=DivIcon(
        icon_size=(1000,36),
        icon_anchor=(0,0),
        html='<div style="font-size: 24pt">All Conference Attendees</div>',
        )
    ).add_to(map)

num_attendees=len(df)
...
for i in range(num_attendees):  #Step through all attendees
    #for i in range(20): #debug
    if i % 100 == 0: print(i)

    html = '<i>' + 'Domain: ' + df.Domain[i] +  '<br>' +\
    'Conference Location: ' + df.L_CityState[i] + '<br>' +\
    'Event Year: ' + str(df.EventYear[i]) + '<br>' +\
    'Conference ID: ' + df['shortName (umbrella)'][i] + '<br>' +\
    'University Affiliate: ' + df['org cleaned up'][i] + '<br>' +\
    'Distance Traveled  (miles): ' + str(int(df['DistanceTraveled'][i])) + '<br></i>'

    iframe = folium.IFrame(html=html, width=700, height=200)
    popup = folium.Popup(iframe, max_width=2650)
    #popup ='hey'

    fgCon.add_child(folium.CircleMarker(location=[df['Lat'][i],df['Long'][i]],color='red',fill_color='red',radius=2,popup=popup))
...
map.save("Map_of_AllAttendees.html")
